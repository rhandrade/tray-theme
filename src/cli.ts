...
export function run() {
    ...
    program
        ...
        .action(async (key, password, theme_name, theme_base) => {
            ...
            log(
                chalk`[${getCurrentLocalteTime()}] {blue [Processing]} Creating theme {magenta ${theme_name}} on store...`
            );
        ...
        .action(async (files) => {
            ...
            if (!assets.length) {
                ...
                if (!themeAssetsResults.success) {
                    log(
                        chalk`[${getCurrentLocalteTime()}] {red [Fail]} Error from api': ${themeAssetsResults.message}.`
                    );
                    process.exit();
                }
    ...
    program
        ...
        .action(async (files: string[]) => {
            ...
            console.log(
                chalk`[${getCurrentLocalteTime()}] Upload process finished. | Sent ${successAssets} files with success. | ${errorAssets} files could not be sent.`
            );
        });
    ...
    program.command('watch').action(async () => {
        ...
        watcher
            ...
            .on('addDir', () => {
                console.log(
                    chalk`[${getCurrentLocalteTime()}] {yellow [Warn]} Creating empty directory is not supported by Tray API. Ignoring....`
                );
            })

            .on('unlinkDir', () => {
                console.log(
                    chalk`[${getCurrentLocalteTime()}] {yellow [Warn]} Deleting directory is not supported by Tray CLI API. Please delete using admin panel. Ignoring....`
                );
            })
            .on('error', (error) => {
